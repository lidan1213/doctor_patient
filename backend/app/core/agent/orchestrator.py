...
class Orchestrator:
    """ReAct-based orchestrator: Intent → ReAct loop (RAG/MCP/Memory) → Generate."""
    ...
    async def run(self, message: str, role: str, role_value: str,
        ...
        # Step 1: Intent classification (lightweight, used for response styling)
        ...
        # Step 3: Emit reasoning steps (for doctor-facing transparency)
        yield {
            "type": "reasoning_steps",
            "steps": [s.to_dict() for s in react_result.steps],
        }

        # Step 4: Build context from ReAct observations
        search_context = self._extract_search_results(react_result)

        # Step 5: Annotate citations if we have search results

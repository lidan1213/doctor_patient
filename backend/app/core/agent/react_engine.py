...
class ReActEngine:
    ...
    async def run(self, message: str, *, role: str = "patient",
        ...
        # Consecutive failure tracking: same tool fails twice → auto-skip
        ...
        for i in range(MAX_ITERATIONS):
            ...
            if observation.startswith("[工具超时]") or observation.startswith("[工具错误]"):
                if action == prev_tool:
                    consecutive_failures += 1
                else:
                    consecutive_failures = 1
                if consecutive_failures >= 2:
                    observation += "\n\n⚠️ 该工具连续两次执行失败，系统已自动跳过。请使用其他工具或直接回答用户问题。"
                prev_tool = action
            else:
                prev_tool = None
                consecutive_failures = 0

...
from app.core.knowledge import search as kb_search, init as kb_init
...
class HybridRetriever:
    ...
    async def search(self, query: str, collection: str, top_k: int = 10) -> list[dict]:
        ...
        # If both failed, fall back to FTS5 knowledge base
        if not bm25_docs and not vector_docs:
            try:
                kb_docs = kb_search(query, collection, top_k)
                if kb_docs:
                    logger.info(f"FTS5 fallback returned {len(kb_docs)} results for {collection}")
                    return kb_docs, []
            except Exception as e:
                logger.warning(f"FTS5 fallback search also failed: {e}")

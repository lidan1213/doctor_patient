...
MOCK_CASES: dict[int, list[dict]] = {
    1: [
        ...
        },
        {
            ...
            "chief_complaint": "血压升高1月",
            "diagnosis": "原发性高血压",
            "department": "心内科",
            "attending_doctor": "王医生",
            "allergies": [],
            "procedures": ["24h动态血压监测"],
...
class PatientRecordModule(BaseMCPModule):
    ...
    async def _query_case(self, user_id: int, _params: dict) -> dict:
        cases = MOCK_CASES.get(user_id, [])
        return {"patient_id": user_id, "cases": cases, "total": len(cases)}

    async def _query_visit(self, user_id: int, params: dict) -> dict:
        visits = MOCK_VISITS.get(user_id, [])
        start_date = params.get("start_date")
        end_date = params.get("end_date")
        if start_date:
            visits = [v for v in visits if v["date"] >= start_date]
        if end_date:
            visits = [v for v in visits if v["date"] <= end_date]
        return {"patient_id": user_id, "visits": visits, "total": len(visits)}

    async def _query_prescription(self, user_id: int, _params: dict) -> dict:
        prescriptions = MOCK_PRESCRIPTIONS.get(user_id, [])
        return {"patient_id": user_id, "prescriptions": prescriptions, "total": len(prescriptions)}
